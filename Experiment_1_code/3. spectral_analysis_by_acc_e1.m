...
%% 2) Compute spectra per subject/condition
...
exportgraphics(gcf, fullfile(outDir,'sig_oscillations-acc_e1.pdf'), ...
...
pdfFile = fullfile(outDir, 'Peak_frequency-acc_e1.pdf');
...
for i = 1:numel(subjects)
    ...
    for ci = 1:numel(conditions)
        x = Ti.respcorrect(strcmp(Ti.condition, conditions{ci}));
        if ~isempty(x); ACC_mean(i,ci) = mean(x,'omitnan'); end
    end
end
...
T = T(:, {'subject','sub_id','condition','hemi','hand','peak_z','ACC_mean'});
T = sortrows(T, {'condition','sub_id'});
outLongFile = fullfile(outDir, 'z_peak_fre_acc_long_e1.xlsx');
