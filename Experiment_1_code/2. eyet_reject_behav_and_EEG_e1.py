...
n_trials_max = np.zeros(len(obs_all), dtype = int)
trial_present_all = []
...
for obs_ind, obs_i in enumerate(obs_all):
	...
	for ind, log_fname in enumerate(log_files):
		log_data = np.loadtxt(log_fname, dtype = str)
		if not ind: log_all_data = log_data[1:, :]
		else: log_all_data = np.concatenate([log_all_data, log_data[1:, :]], axis = 0)

	log_block_all = log_all_data[:, 1].astype(int)
	log_trial_all = log_all_data[:, 2].astype(int)
	responded_mask_log_all.append(log_all_data[:, -3].astype(float) != -1)
	...
	for block_ind, block in enumerate(usable_blocks):
		...
		logeyet_data = np.loadtxt(logeyet_fname, dtype = str)
		if not block_ind:
			logeyet_all_data = logeyet_data[1:, :]
		else:
			logeyet_all_data = np.concatenate([logeyet_all_data, logeyet_data[1:, :]], axis = 0)
	logeyet_block_all = logeyet_all_data[:, 1].astype(int)
	logeyet_trial_all = logeyet_all_data[:, 2].astype(int)
	responded_mask_logeyet.append(logeyet_all_data[:, -3].astype(float) != -1)
	...
	if os.path.exists(log_data_path + 'eeg_fix/'):
		...
		for ind, log_fname in enumerate(log_files):
			logeeg_data = np.loadtxt(log_fname, dtype = str)
			if not ind:
	...
	logeeg_block_all = logeeg_all_data[:, 1].astype(int)
	logeeg_trial_all = logeeg_all_data[:, 2].astype(int)
	...
	trial_present = np.zeros(shape = [n_trials_max[obs_ind], 2], dtype = bool)
	...
	responded_mask = logdata_all[:, -3].astype(float) != -1
	logdata_all_clean = logdata_all[responded_mask, :]

	# get useful stuff from log
	isds_all = logdata_all_clean[:, 4].astype(int)

	n_trials = block_n_all_clean.shape[0]
	rej_trial_eyet = np.zeros(n_trials, dtype = bool)
	...
	if obs_i not in [33]:
		...
		maskk = responded_mask_log_all[obs_ind] & trials_all_info_all[obs_ind][:, 0].astype(bool)
	...
	# clean eeg data
	if obs_i not in [33]:
		eeg_trials_pres = trials_all_info_all[obs_ind][:, 1].astype(bool)
...
########################################################################################################################
# Convert cleaned-behavior .npy structs -> CSV
########################################################################################################################

SOURCE_ROOT = Path(base_path)
OUTPUT_DIR  = SOURCE_ROOT / "beh_clean_data"
COLUMNS = ["observer","block","trial","instr_type","ISD","stimcombi","response","resptime","resptime2","respcorrect"]
OUTPUT_DIR.mkdir(parents=True, exist_ok=True)

exclude = {5, 9, 15, 16, 33}

files = sorted(SOURCE_ROOT.rglob(
    f"behav/obs_*_behav_data_eyet_thresh{fix_thresh_indeg:.1f}deg_struct.npy"
))

for f in files:
    m = re.search(r"obs_(\d+)", f.stem)
    if (not m) or (int(m.group(1)) in exclude):
        continue

    d = np.load(f, allow_pickle=True).item()
    df = pd.DataFrame({k: pd.Series(d.get(k, [])) for k in COLUMNS}, columns=COLUMNS)

    # bytes -> str 
    for k in COLUMNS:
        if df[k].dtype == object:
            df[k] = df[k].map(lambda x: x.decode() if isinstance(x, (bytes, bytearray)) else x)

    obs_tag = re.search(r"obs_\d+", f.name).group(0)
    df.to_csv(OUTPUT_DIR / f"{obs_tag}_beh_clean.csv", sep=";", index=False, encoding="utf-8-sig")
