...
for obs_i in obs_all:
	...
	for block in np.arange(1, 9):
		...
		if os.path.exists(fp):
			...
			trigs_smi = processed[1:, -3].astype(float)
			...
			raw_trig_timep = processed[1:, 0].astype(float)
			...
			mrk_timep = processed_mrk[:, 2].astype(int)/500.
			...
			trigs_noz = trigs[trigs > 0].astype(int)
			...
			new_raw_trig_chan = np.zeros(len(raw_trig_timep), dtype = int)
			new_raw_trig_chan[corresp_eyet_timep_inds] = mrks_as_trigs.astype(int)

			# recreate EyeT data with matched EEG triggers.
			recon_eyet_trigs = np.vstack([raw_trig_timep, new_raw_trig_chan,
				processed[1:, 21].astype(float), processed[1:, 22].astype(float)])
...
pre_samples = np.ceil(np.abs(epoch_tstart) / (1./srate)).astype(int)
post_samples = np.ceil(np.abs(epoch_tend) / (1./srate)).astype(int)
...
for obs_i in obs_all:
	...
	for file_n in eyetrecon_files:
		...
		epochs_len = np.zeros(len(trial_start_trigs_inds), dtype = int)
		for trial_ind, t in enumerate(timep_trial_start):
			...
			epoch_mask = np.zeros(len(eyet_timep_inSec), dtype = bool)
			...
			trig_all.append(recon_eyet_trigs[epoch_mask, 1].astype(int))
		...
		log_data = np.loadtxt(log_fname[0], dtype = str)
		...
		responded_mask = log_data[1:, -3].astype(float) != -1

		# concatenate blocks and create array for block-"numbering" each trial
		block_info = block * np.ones(neyet_trials, dtype = int)
		if first_block:
			first_block = False
