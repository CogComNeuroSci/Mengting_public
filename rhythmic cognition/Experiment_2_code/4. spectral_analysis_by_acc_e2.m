...
srate        = 20;
nARSurr      = 1000;
category_map = repmat({'unknown'}, 16, 1);
...
files    = dir(fullfile(cleanDir, 'obs_*_clean.csv'));
subjects = sort(regexprep({files.name}, '_clean\.csv$', '', 'once')); 
...
for i = 1:numel(subjects)
    ...
    T.respcorrect = double(T.respcorrect);
    T.ISI_ms = 1200 + 50 * double(T.ISI);
    T.condition = category_map(T.instr_type + 1);
    allData{i, 1} = T;
end

%% 2. Compute spectra per subject and condition 
specc_dev = nan(numel(subjects), numel(conditions), pad/2);
specc_amp = nan(numel(subjects), numel(conditions), pad/2);

for i = 1:numel(subjects)
    combined = allData{i, 1};  
    ...
    for ci = 1:numel(conditions)
        ...
        % aggregate accuracy by ISI 
        aggC = groupsummary(Tc, 'ISI_ms', 'mean', 'respcorrect');

        % AR surrogate spectrum by proportion correct
        [dC, frex, aC] = ARSurrSpectByPs(aggC.mean_respcorrect, srate, nARSurr, pad);
...
title(ax,'Peak frequency by Accuracy','FontSize',6);
box(ax,'off');

if ~exist(dataDir,'dir'); mkdir(dataDir); end
pdfFile = fullfile(dataDir, 'Peak_frequency-acc.pdf');
exportgraphics(gcf, pdfFile, 'ContentType','vector', 'BackgroundColor','none');


%% --- prepare for LMM ---
validSubjects = cellstr(string(subjects(validRowAny)))';
condNames = {'SS','SSiDS','DSi'};

T_z = array2table(z_plot, 'VariableNames', condNames);
T_z.subject = validSubjects;
...
acc_mean = nan(numel(subjects), numel(conditions));
for i = 1:numel(subjects)
    Ti = allData{i,1}; if isempty(Ti), continue; end
    for ci = 1:numel(conditions)
        m = Ti.respcorrect(strcmp(Ti.condition, conditions{ci}));
        if ~isempty(m), acc_mean(i,ci) = mean(m, 'omitnan'); end
    end
end

A = array2table(acc_mean(validRowAny,:), 'VariableNames', condNames);
A.subject = string(subjects(validRowAny))';
A = movevars(A, 'subject', 'Before', 1);
A = stack(A, condNames, 'NewDataVariableName','acc_mean', 'IndexVariableName','condition');
A.subject   = string(A.subject);
A.condition = string(A.condition);

T = innerjoin(T, A, 'Keys', {'subject','condition'});
T = T(:, {'subject','sub_id','condition','peak_z','acc_mean'});
T = sortrows(T, 'sub_id');

outLongFile = fullfile(dataDir, 'z_peak_fre_acc_long_e2.xlsx');
